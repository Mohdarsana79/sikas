...
<script>
    $(document).ready(function() {
            ...
            $('#transactionModal').on('show.bs.modal', function() {
                ...
                // Reset tombol hapus (sembunyikan untuk card pertama)
            ...
            $(document).on('change', '.rekening-select', function() {
                ...
                if (rekeningId && kegiatanId) {
                    ...
                    // Load uraian berdasarkan rekening DAN kegiatan (WAJIB KEDUANYA)
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response.success) {
                                if (response.data && response.data.length > 0) {
                                    response.data.forEach(function(uraian) {
                                        if (uraian.dapat_digunakan) {
                                            uraianContainer.append(`<div class="uraian-item card mb-2">
                                                <div class="card-body">
                                                    <div class="form-check mb-2">
                                                        <input class="form-check-input uraian-checkbox" type="checkbox" value="${uraian.id}" id="uraian-${uraian.id}" data-kegiatan="${uraian.kode_id}">
                                                        <label class="form-check-label" for="uraian-${uraian.id}">
                                                            <strong>${uraian.uraian}</strong> - Rp ${formatRupiah(uraian.harga_satuan)}/${uraian.satuan}
                                                        </label>
                                                    </div>
                                                    <div class="row">
                                                        <div class="col-md-6">
                                                            <label class="form-label">Jumlah</label>
                                                            <input type="number" class="form-control form-control-sm jumlah-input" placeholder="Jumlah" min="1" value="1" max="${uraian.sisa_volume}">
                                                            <small class="text-muted">Sisa volume: ${uraian.sisa_volume} ${uraian.satuan}</small>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <label class="form-label">Harga Satuan</label>
                                                            <input type="text" class="form-control form-control-sm harga-input" placeholder="Harga" value="${formatRupiah(uraian.harga_satuan)}" oninput="formatCurrency(this)" readonly>
                                                        </div>
                                                    </div>
                                                    <div class="mt-2">
                                                        <strong>Subtotal: <span class="subtotal">Rp ${formatRupiah(uraian.harga_satuan)}</span></strong>
                                                    </div>
                                                </div>
                                            </div>`);
                                        }
                                    ...
                                    if (uraianContainer.children().length === 0) {
                                        uraianContainer.html(
                                            '<p class="text-muted">Tidak ada uraian yang tersedia untuk kombinasi kegiatan dan rekening ini.</p>'
                                        );
                                    }
                                } else {
                                    uraianContainer.html(
            ...
            // Event handler untuk perubahan checkbox uraian, jumlah, atau harga
            $(document).on('change input', '.uraian-checkbox, .jumlah-input, .harga-input', function() {
            ...
            function updateUraianSubtotal(uraianItem) {
                ...
                const subtotalElement = uraianItem.find('.subtotal');
                ...
                if (checkbox.is(':checked')) {
                    ...
                    subtotalElement.text(`Rp ${formatRupiah(subtotal)}`);
                } else {
            ...
            $('#tambahKegiatan').click(function() {
                const kegiatanCount = $('.kegiatan-card').length + 1;
                const newKegiatanCard = `
                <div class="card mb-3 kegiatan-card" data-kegiatan-index="${kegiatanCount}">
                    <div class="card-header bg-info text-white d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Kegiatan ${kegiatanCount}</h5>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                <label class="form-label">Rekening Belanja</label>
                                <select class="form-select form-select-sm rekening-select" name="kode_rekening_id[]" required>
                                    <option value="">Pilih Rekening Belanja</option>
                                </select>
                            </div>
                        </div>
            ...
            `;
                ...
                // Populate kegiatan select untuk card baru
                populateKegiatanSelect();

                // Tampilkan tombol hapus untuk semua card kecuali yang pertama
            ...
            // Fungsi untuk memformat input currency
            window.formatCurrency = function(input) {
                ...
                if (value) {
                    ...
                    // Trigger event change untuk menghitung ulang subtotal
