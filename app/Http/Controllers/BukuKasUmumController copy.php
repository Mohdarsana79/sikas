...
class BukuKasUmumController extends Controller
{
    ...
    public function getRekapanBkuAjax(Request $request)
    {
        try {
            ...
            if ($tabType === 'Bank') {
                ...
                $html = $this->generateBeritaAcaraHtml($penganggaran, $tahun, $bulan, $bulanAngka);
            }else {
     ...
     */
    private function generateBeritaAcaraHtml($penganggaran, $tahun, $bulan, $bulanAngka)
    {
        try {
    ...
    /**
     * Get data Berita Acara Pemeriksaan Kas
     */
    public function getBeritaAcaraData($tahun, $bulan)
    {
        try {
            ...
            // Ambil data dari BKP Registrasi untuk perhitungan
            $regData = $this->getDataForBeritaAcara($penganggaran->id, $tahun, $bulan, $bulanAngka);

            $html = view('laporan.partials.ba-pemeriksaan-kas-table', $regData)->render();
    ...
    /**
     * Generate PDF Berita Acara Pemeriksaan Kas
     */
    public function generateBeritaAcaraPdf($tahun, $bulan)
    {
        try {
            ...
            $pdf = PDF::loadView('laporan.ba-pemeriksaan-kas-pdf', $data);
    ...
    /**
     * Helper method untuk mendapatkan data Berita Acara - VERSI DIPERBAIKI
     */
    private function getDataForBeritaAcara($penganggaran_id, $tahun, $bulan, $bulanAngka)
    {
        try {
            Log::info('=== GET DATA FOR BERITA ACARA ===', [
                'penganggaran_id' => $penganggaran_id,
                'tahun' => $tahun,
                'bulan' => $bulan,
                'bulan_angka' => $bulanAngka
            ...
            if (!$sekolah) {
                Log::warning('Data sekolah tidak ditemukan');
            }

            if (!$penganggaran) {
                Log::warning('Data penganggaran tidak ditemukan', ['penganggaran_id' => $penganggaran_id]);
            }

            // Hitung saldo kas dari Pembantu
            $saldoKas = $this->getSaldoKasFromPembantu($penganggaran_id, $tahun, $bulan, $bulanAngka);
            Log::info('Saldo Kas dari Pembantu:', ['saldoKas' => $saldoKas]);

            // Data uang kertas berdasarkan saldo kas
            $uangKertas = $this->getDenominasiUangKertas($saldoKas);
            $totalUangKertas = $this->hitungTotalUangKertas($uangKertas);
            Log::info('Total Uang Kertas:', ['totalUangKertas' => $totalUangKertas]);

            // Data uang logam
            $uangLogam = $this->getDenominasiUangLogam();
            $totalUangLogam = $this->hitungTotalUangLogam($uangLogam);
            Log::info('Total Uang Logam:', ['totalUangLogam' => $totalUangLogam]);

            // Total uang kertas + logam
            $totalUangKertasLogam = $totalUangKertas + $totalUangLogam;
            Log::info('Total Uang Kertas + Logam:', ['total' => $totalUangKertasLogam]);

            // Saldo Bank dari BKP Bank
            $saldoBank = $this->hitungSaldoAkhirBkpBank($penganggaran_id, $tahun, $bulan);
            Log::info('Saldo Bank:', ['saldoBank' => $saldoBank]);

            // Total Kas = Uang Kertas/Logam + Saldo Bank
            $totalKas = $totalUangKertasLogam + $saldoBank;
            Log::info('Total Kas:', ['totalKas' => $totalKas]);

            // Ambil total penerimaan dan pengeluaran dari tab Umum untuk saldo buku
            $dataUmum = $this->getDataFromBkpUmumCalculation($penganggaran_id, $tahun, $bulan, $bulanAngka);
            $saldoBuku = $dataUmum['totalPenerimaan'] - $dataUmum['totalPengeluaran'];
            Log::info('Saldo Buku:', [
                'totalPenerimaan' => $dataUmum['totalPenerimaan'],
                'totalPengeluaran' => $dataUmum['totalPengeluaran'],
                'saldoBuku' => $saldoBuku
            ...
            Log::info('Perbedaan:', ['perbedaan' => $perbedaan]);

            // Tanggal akhir bulan
            $tanggalAkhirBulan = Carbon::create($tahun, $bulanAngka, 1)->endOfMonth();
            $namaHariAkhirBulan = $this->getNamaHariIndonesia($tanggalAkhirBulan);

            Log::info('=== DATA BERITA ACARA SELESAI DIHITUNG ===');
    ...
    /**
     * Ambil data dari perhitungan BKP Registrasi untuk konsistensi
     */
    private function getDataFromBkpRegCalculation($penganggaran_id, $tahun, $bulan, $bulanAngka)
    {
        try {
            // Gunakan method yang sama seperti di BKP Registrasi
            $regData = $this->generateRegHtml(Penganggaran::find($penganggaran_id), $tahun, $bulan, $bulanAngka);

            // Parse data dari BKP Registrasi
            $penganggaran = Penganggaran::find($penganggaran_id);
            $sekolah = Sekolah::first();
            ...
            Log::error('Error getDataFromBkpRegCalculation: ' . $e->getMessage());
