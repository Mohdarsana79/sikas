...
class RkasManager {
    ...
    updateTotalAnggaran() {
        ...
        document.querySelectorAll('.anggaran-bulan-card').forEach((card) => {
    ...
    */
    ...
    /**
     * Handle sisipkan button click
     */
    handleSisipkanButtonClick(button) {
        ...
        // Set nilai form
        this.setSisipkanFormValues(validKodeId, program, kegiatan, validRekeningId, rekeningDisplay, bulan);
    ...
    /**
     * Set sisipkan form values
     */
    setSisipkanFormValues(kodeId, program, kegiatan, rekeningId, rekeningDisplay, bulan) {
        // Set hidden values
    ...
    calculateSisipkanTotal() {
        ...
        document.getElementById('sisipkan_total_display').textContent = 'Rp ' + this.formatNumber(total);
    }

    /**
     * Submit sisipkan form
     */
    submitSisipkanForm(form) {
    ...
    /**
     * Validate sisipkan form
     */
    validateSisipkanForm() {
        const requiredFields = [
            'sisipkan_uraian',
            'sisipkan_jumlah', 
            'sisipkan_satuan',
                        ...
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="actionDropdown${item.id}">
                            ...
                            <li>
                                <a class="dropdown-item sisipkan-btn" href="#" 
                                    ...
                                    data-program="${this.escapeHtml(item.program_kegiatan)}"
                                    data-kegiatan="${this.escapeHtml(item.kegiatan)}"
                                    data-rekening-id="${item.kode_rekening_id}"
                                    data-rekening-display="${this.escapeHtml(item.rekening_belanja)}">
    ...
    setupEventListeners() {
        ...
        document.addEventListener('input', (e) => {
            ...
            if (e.target.id === 'sisipkan_harga_satuan' || e.target.id === 'sisipkan_jumlah') {
                this.calculateSisipkanTotal();
            }
        });

        // Form submission - PERBAIKAN: gunakan arrow function
        document.getElementById('tambahRkasForm')?.addEventListener('submit', (e) => {
        ...
        // Sisipkan form submission - PERBAIKAN: gunakan arrow function
        document.getElementById('sisipkanRkasForm')?.addEventListener('submit', (e) => {
            e.preventDefault();
            this.submitSisipkanForm(e.target);
        });
        ...
        document.addEventListener('click', (e) => {
            if (e.target.closest('.sisipkan-btn')) {
                e.preventDefault();
                try {
                    this.handleSisipkanButtonClick(e.target.closest('.sisipkan-btn'));
                } catch (error) {
